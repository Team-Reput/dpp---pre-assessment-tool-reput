...
export class Sustainability {
  ...
  readonly currentStep = 8;

  form: FormGroup;
  isSubmitting = false;
  submitError = '';

  // Used by the template's @for loop — must stay an ARRAY
  ...
  // Used only when building the payload — short code → full label
  ...
  onContinue(): void {
    ...
    this.sustainabilityService.saveSustainabilityData(payload).subscribe({
      next: (response) => {
        this.isSubmitting = false;
        console.log('Sustainability data saved:', response);
