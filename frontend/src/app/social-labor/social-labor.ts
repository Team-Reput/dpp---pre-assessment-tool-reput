...
import { SocialLaborService } from './social-labor.service';
import { AssessmentStateService } from '../shared/assessment-state.service';

@Component({
  selector: 'app-social-labor',
  standalone: true,
  imports: [ReactiveFormsModule],
  templateUrl: './social-labor.html',
  styleUrl: './social-labor.scss',
})
export class SocialLabor {
  readonly totalSteps = 12;
  readonly currentStep = 10;
  ...
  constructor(
    ...
    private socialLaborService: SocialLaborService,
  ...
  onContinue(): void {
    ...
    this.socialLaborService.saveSocialLaborData(payload).subscribe({
      next: (response) => {
        this.isSubmitting = false;
        console.log('Social & labor data saved:', response);
